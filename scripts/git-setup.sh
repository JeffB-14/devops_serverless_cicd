...
source scripts/deployment-variables-demo.sh

# Get parameters from stack
USER=$(aws cloudformation describe-stacks --stack-name ${STACK_NAME} --profile ${PROFILE} --query "Stacks[*].Outputs[?OutputKey=='IAMUserName'].OutputValue" --output text)
CODECOMMIT_SSH_URL=$(aws cloudformation describe-stacks --stack-name ${STACK_NAME} --profile ${PROFILE} --query "Stacks[*].Outputs[?OutputKey=='RepoCloneUrlSsh'].OutputValue" --output text)
...
SSH_PUB_KEY_ID=$(aws --profile ${PROFILE} iam upload-ssh-public-key --user-name $USER --ssh-public-key-body file://$SSH_KEY_LOC.pub --query "SSHPublicKey.SSHPublicKeyId" --output text)
ssh-add $SSH_KEY_LOC
